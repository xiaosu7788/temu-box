...
async function submit() {
  if (username.value.trim().length < 3 || password.value.length < 8) {
    ElMessage.warning(registerMode.value ? '用户名至少 3 位，密码至少 8 位' : '请输入用户名和密码')
    return
  }
  loading.value = true
  try {
  ...
  <main class="auth-shell">
    <section class="auth-panel">
      ...
      <el-form @submit.prevent="submit">
        <el-form-item><el-input v-model="username" placeholder="用户名" size="large" :prefix-icon="UserIcon" autocomplete="username" /></el-form-item>
        <el-form-item v-if="registerMode"><el-input v-model="displayName" placeholder="显示名称（可选）" size="large" /></el-form-item>
        <el-form-item><el-input v-model="password" type="password" show-password placeholder="密码" size="large" :prefix-icon="Lock" autocomplete="current-password" @keyup.enter="submit" /></el-form-item>
